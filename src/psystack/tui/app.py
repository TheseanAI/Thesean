"""PsyStackApp — case-based investigation workspace."""
...
class TuiStageObserver:
    ...
    def __init__(self, app: PsyStackApp) -> None:
        self.app = app
...
class PsyStackApp(App):
    ...
    def __init__(self, explicit_workspace: Path | None = None) -> None:
        ...
        self.register_theme(psystack_theme)
        self.theme = "psystack"

        self.state = AppState()
        self.backend = TuiBackendService()
        self.actions_handler = PsyStackActions(self, self.backend)
    ...
    #     from psystack.adapters.f1.live_viewer import LiveF1ViewerProcess
    #     ...
    ...
    def show_error(self, title: str, message: str) -> None:
        from psystack.tui.screens.error_modal import ErrorModal
    ...
    def _open_run_builder(self) -> None:
        ...
        for r in load_recent_cases():
            if r.project:
                p = Path(r.project) / ".psystack" / "cases" / r.case
    ...
    def _open_attribution(self) -> None:
        from psystack.tui.screens.attribution import AttributionWorkspaceScreen
    ...
    def on_run_builder_screen_case_created(self, event: RunBuilderScreen.CaseCreated) -> None:
        ...
        if ctx.adapter and ctx.project_root:
            ...
            from psystack.pipeline.workspace import create_workspace_dirs, write_workspace_state
