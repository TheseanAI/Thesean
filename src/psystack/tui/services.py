...
from psystack.cli.wizard.discovery import (
...
class TuiBackendService:
    ...
    def create_workspace(self, workspace: Path, case: Case, adapter_name: str, repo: Path) -> None:
        ...
        Creates directory structure, writes case.json + psystack.toml + workspace_state.json.
        """
    ...
    def list_cases(self, project_root: Path) -> list[Path]:
        """List valid case directories under .psystack/cases/."""
        cases_dir = project_root / ".psystack" / "cases"
        if not cases_dir.is_dir():
            return []
        cases: list[Path] = []
        for child in sorted(cases_dir.iterdir()):
            if child.is_dir() and ((child / "psystack.toml").exists() or (child / "case.json").exists()):
                cases.append(child)
        return cases
    ...
    def write_workspace_config(self, workspace: Path, adapter_name: str, repo: Path) -> None:
        """Write psystack.toml to workspace so detection and RunContext can find it."""
        import tomli_w
        ...
        (workspace / "psystack.toml").write_text(tomli_w.dumps(config))
    ...
    def _resolve_translator_from_workspace(self, workspace: Path) -> Any:
        """Resolve a SignalTranslator from workspace psystack.toml config."""
        try:
            import tomllib
        except ModuleNotFoundError:
            import tomli as tomllib  # type: ignore[no-redef]
        toml_path = workspace / "psystack.toml"
        ...
        try:
            ...
            if not adapter_type or not repo:
                return None
            from psystack.cli.wizard.discovery import load_factory
    ...
    def run_analysis(self, workspace: Path, translator: Any = None) -> OutcomeSummary:
        ...
        try:
            ...
            import logging
            logging.getLogger("psystack.pipeline").warning("Event extraction failed: %s", exc)
    ...
    def generate_report_from_artifacts(self, workspace: Path) -> Path:
        ...
        template_dir = files("psystack.reporting") / "templates"
